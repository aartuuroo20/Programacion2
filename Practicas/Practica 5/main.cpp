...
void _jugar(Jugador& a, Jugador& b, Bolsa& B,int acum1, int acum2){
    ...
    if(!a.getPlantado()){
        ...
        cin>>aux;
        if(aux!=1)a.setPlantado(true);
    }
    if(B.quedanBolas()&&!a.getPlantado()){
        aux=B.sacarBolas();
        cout<<"bola: "<<aux<<endl;
        acum1=acum1+aux;
    ...
    if(!b.getPlantado()){
        ...
        cin>>aux;
        if(aux!=1)b.setPlantado(true);
    }
    if(B.quedanBolas()&&!b.getPlantado()){
        aux=B.sacarBolas();
        cout<<aux<<endl;
        cout<<"bola: "<<aux<<endl;
        acum2=acum2+aux;
    ...
    if(!a.getPlantado()||!b.getPlantado())_jugar(a,b,B,acum1,acum2);
    else{
        //calculo quien gana
        int pa=0;
        int pb=0;
